...
export default async function getValidUser(ignoreCache = false, ...selects: string[]): Promise<IUser> {
    ...
    for (let i = 0; i < allUsers.length; i++) {
        ...
        try {
            ...
            Logger.write(`Calendar-beforeAll: Failed looking up user '${testUserName}'`, LogLevel.Verbose);
        }
    }

    if (cachedValidUser === null) {
        throw Error(`Could not find a valid user in the first ${usersToCheck} results.`);
    }
