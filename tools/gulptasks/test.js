...
gulp.task("test", ["clean", "build:test", "_istanbul:hook"], () => {
    ...
    if (yargs.packages || yargs.p) {
        ...
        if (yargs.single || yargs.s) {
            ...
            paths.push(path.resolve(`./build/testing/test/${args.packages[0]}/`, (yargs.single || yargs.s) + ".js"));
        } else {
            paths = args.packages.map(p => `./build/testing/test/${p}/**/*.js`);
        }

    } else {
        paths.push("./build/testing/**/*.js");
    }
