...
declare module "./spqueryable" {
    interface _SPCollection {
        ...
        **/
        where: (cb: (builder: InitClause<any>) => void) => this;
    }
}

_SPCollection.prototype.where = function <T extends _SPCollection>(this: T, cb: (this: T, builder: InitClause<T>) => void): this {

    const w = new _Where(this);

    cb.bind(this)(w);

    if ((<any>w).clauseBuilder.length > 0) {
        throw Error("The where clause is not properly closed.");
    }

    this.filter(w.toString());

    return this;
}

// these are the methods that begin a clause
type InitClause<T extends _SPCollection> = Pick<_Where<T>, "str" | "num" | "date">;

// these are the methods that 
type CloseClause<T extends _SPCollection, ValueType> = Pick<_Where<T, ValueType>, "eq" | "neq" | "gt" | "lt" | "gte" | "lte">;
...
class _Where<T extends _SPCollection, ValueType = any> {
    ...
    public str(fieldName: string, options?: { escape?: boolean }): CloseClause<T, string> {
        return this.openClause<string>(fieldName, (v: string) => {
    ...
    /**
     * Below here are the clause-closing methods exposed in type CloseClause
     * 
     * If you add methods here, be sure to update CloseClause to pick them so they are properly exposed
     */
