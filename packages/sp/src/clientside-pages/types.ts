...
export class _ClientSidePage extends _SharePointQueryable implements _IClientSidePage {
    ...
    public save(publish = true): Promise<boolean> {
        ...
        if (!this.json.IsPageCheckedOutToCurrentUser) {
            promise = promise.then(_ => spPost(_ClientSidePage.initFrom(this, `_api/sitepages/pages(${this.json.Id})/checkoutpage`)));
        }
        ...
        promise = promise.then(_ => {
            ...
            return spPost<boolean>(updater, { body: jsS(saveBody) });
        });

        if (publish) {
            promise = promise.then(_ => spPost(_ClientSidePage.initFrom(this, `_api/sitepages/pages(${this.json.Id})/publish`))).then(r => {
                if (r) {
